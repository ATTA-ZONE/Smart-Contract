...
contract VendingMachine is Ownable {
    ...
    mapping(uint256 => uint256) public _commodityAmounts; //剩余库存
    event PriceUpdate(uint256 indexed tokenId, uint256 price, uint256 amounts);
    ...
    function _listToken(uint256 _tokenId, uint256 _price, uint256 _amounts) internal {
        _commodityPrices[_tokenId] = _price;
        _commodityAmounts[_tokenId] = _amounts;
        emit PriceUpdate( _tokenId, _price, _amounts);
    }
    
    function safeListToken(uint256 _tokenId, uint256 _price, uint256 _storage) public onlyOwner {
        _listToken(_tokenId, _price, _storage);
    }
    
    function _buyToken(address _buyer, uint256 _tokenId, uint256 _quantity) internal {
        require(_quantity < _commodityAmounts[_tokenId], "out of stock");
        _commodityAmounts[_tokenId] -= _quantity;
