...
contract ERC721Mintable is ERC721 ,Ownable{
    ...
    event WEIGHT(uint256 _value, uint256 indexed _id);
    ...
    function setWeight(uint256 _weight, uint256 _id) external onlyOwner {
        //require(msg.sender == creators[_id]);
        weights[_id] = _weight;
        emit WEIGHT(_weight, _id);
    }
    
    function setBatchWeight(uint256 _weight, uint256[] calldata _id) external onlyOwner {
        //require(msg.sender == creators[_id]);
        for (uint256 i = 0; i < _id.length; ++i) {
            uint256 id = _id[i];
            weights[id] = _weight;
            emit WEIGHT(_weight, id);
        }
    }
    
    function weight(uint256 _id) public view returns (uint256 weight) {
        return weights[_id];
    }
